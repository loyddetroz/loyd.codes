...
const resize = (foreground: Canvas, offscreen: Canvas) => {
  const {innerWidth, innerHeight} = window;
...
window.addEventListener(
  'resize',
  () => resize(foreground, offscreenCanvas),
  false
);
...
techList.forEach(tech => {
  ...
  item.className =
    'h-min w-min rounded-md bg-[#4575F6] text-center text-white py-1 px-2 flex items-center';
